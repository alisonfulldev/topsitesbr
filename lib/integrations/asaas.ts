...
function getApiKey(): string {
  ...
  const envCodes = Array.from(rawEnv).map(c => c.charCodeAt(0)).join(',')
  console.log(`[Asaas] key len=${key.length} prefix=${key.substring(0, 20)} env="${rawEnv}" codes=[${envCodes}] url=${getBaseUrl()}`)
  return key
}
