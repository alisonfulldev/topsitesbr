              ...
              value={
                <span className="flex items-center gap-3 flex-wrap">
                  <span className="text-green-700">
                    {(client.termsAcceptedAt as Date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', timeZone: 'America/Sao_Paulo' })}
                    {' às '}
                    {(client.termsAcceptedAt as Date).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'America/Sao_Paulo' })}
                    {('termsVersion' in client && client.termsVersion)
                      ? <span className="text-gray-400 text-xs ml-1">(v{client.termsVersion as string})</span>
                      : null}
                  </span>
                  <Link
                    href={`/admin/clientes/${client.id}/comprovante`}
                    target="_blank"
                    className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors"
                  >
